...
async function fetchPolyfill() {
  return typeof globalThis.fetch !== "undefined" ? globalThis.fetch : (await import("../../../../lib-QOVOAIFV.mjs")).default;
}
...
var DEFAULT_CANVAS_NODE_COLOR = "#1F1F1F";
var DEFAULT_CANVAS_NODE_TEXT_COLOR = "#EDEDED";
var NODE_COLORS = [
...
var CANVAS_SHAPE_DEFAULT_SIZES = {
...
// trigger/design-agent.ts
var AI_USER_ID = "ghost-ai-agent";
var AI_USER_NAME = "Ghost AI";
var AI_CURSOR_COLOR = "#8b82ff";
var MAX_STATUS_MESSAGES = 8;
var MAX_NODE_WIDTH = 280;
var MAX_NODE_HEIGHT = 180;
var CANVAS_HANDLE_VALUES = ["top", "right", "bottom", "left"];
var CANVAS_SHAPE_VALUES = [
  "rectangle",
  "diamond",
  "circle",
  "pill",
...
function formatError(error3) {
  return error3 instanceof Error ? error3.message : "Unknown error";
}
__name(formatError, "formatError");
async function appendStatus(client, roomId, kind, text) {
  const statusMessage = createAiStatusFeedMessage({ kind, text });
  await client.mutateStorage(roomId, ({ root }) => {
    const current = root.get(AI_STATUS_FEED_ID) ?? root.get("aiStatus") ?? [];
    const nextMessages = [...current, statusMessage].slice(-MAX_STATUS_MESSAGES);
    root.set(AI_STATUS_FEED_ID, nextMessages);
  });
}
__name(appendStatus, "appendStatus");
async function setAiPresence(client, roomId, presence, ttl = 60) {
...
async function getCurrentCanvas(client, roomId) {
  let snapshot = {
    nodes: [],
    edges: []
  };
...
__name(getCurrentCanvas, "getCurrentCanvas");
function buildPrompt(payload, snapshot) {
  return [
...
async function planDesign(payload, snapshot) {
  const result = await generateObject({
    ...
    system: "You are Ghost AI, a senior system architecture designer. Convert user requests into clear canvas updates for a collaborative React Flow diagram. Use only the provided action schema.",
...
__name(planDesign, "planDesign");
function slugifyId(value, fallback) {
  const slug = value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48);
...
function getColorPair(colorName) {
  if (!colorName) {
    return NODE_COLORS[0];
  }
  return NODE_COLORS.find((color) => color.name.toLowerCase() === colorName.toLowerCase()) ?? NODE_COLORS[0];
}
__name(getColorPair, "getColorPair");
...
function normalizeHandle(handle) {
  return handle && CANVAS_HANDLE_VALUES.includes(handle) ? handle : void 0;
}
...
__name(getFallbackPlan, "getFallbackPlan");
...
async function applyDesignPlan(client, roomId, plan) {
  await mutateFlow({ client, roomId }, (flow) => {
    ...
    for (const action of plan.actions) {
      ...
      if (action.type === "deleteNode") {
        ...
        const connectedEdgeIds = flow.edges.filter((edge) => edge.source === nodeId || edge.target === nodeId).map((edge) => edge.id);
...
var designAgentTask = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      const message = formatError(error3);
