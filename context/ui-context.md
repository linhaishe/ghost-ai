...
Both fonts are loaded locally via `next/font/local` from `app/fonts/` and applied as CSS variables on the `<html>` element. The base `body` uses Geist Sans with `antialiased`.
